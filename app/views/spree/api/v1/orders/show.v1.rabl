...
child payments: :payments do
  attributes *payment_attributes
   ...
   begin
    node :payment_url do |payment|
      if !payment.order.paid? && payment.editable? && payment.payment_method && payment.payment_method.respond_to?(:api_payment_url)
        payment.payment_method.api_payment_url
      end
    end
  rescue Exception => e
